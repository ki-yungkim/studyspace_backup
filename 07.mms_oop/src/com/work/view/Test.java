...
public class Test {
	...
	public static void main(String[] args) {
		// 회원 초기화데이터 참고로 해서 회원 5명 객체 생성 
		// 참조변수 타입은 부모타입 : 변수 타형성  //JVM 그림 한번씩 그려보자 
		
		// 부모타입의 참조변수에 일반회원(모든데이터 초기화 생성자) 객체 생성 : 참조변수  dto1, 2
		...
		// 실제 메모리에 생성된 자식객체타입으로 형변환 => 실제 자식객체의 멤버 접근 가능
		GeneralMember g1 = (GeneralMember)dto1; //g1.getMileage(); 사용 가능 
		...
		if (dto1 instanceof AdminMember) {
			...
			//인스턴스 체크 없이 하면 error : java.lang.ClassCastException: com.work.model.dto.GeneralMember cannot be cast to com.work.model.dto.AdminMember at com.work.view.Test.main(Test.java:27)
			a1.getPosition();
		} else {
		...
		// 부모타입의 참조변수에 우수회원(모든데이터 초기화 생성자) 객체 생성 : 참조변수 dto3, 4 
		...
		// 부모타입의 참조변수에 관리자회원(모든데이터 초기화 생성자) 객체 생성 : 참조변수 dto5 
		Member dto5 = new AdminMember("administrator","admin1004","유관순","01012345000","administrator@work.com", "2020-04-01", "A", "선임");
	}
	
	// 재정의 테스트 
	public static void main2(String[] args) {
		Member dto1 = new Member("user01","password01","홍길동","010123451234","user01@work.com");
		System.out.println(dto1); // 어떤 클래스의 toString() 결과?? => 부모 재정의
		// toString() 재정의하지 않는 경우 : com.work.model.dto.Member@15db9742
		// toString() 재정의 : null, null, null, null, null, null, null
		// @Override 주석처리하면 중복정의 
		
		// 일반회원객체 생성
		Member dto2 = new GeneralMember("user02","password02","강감찬","01012345002","user02@work.com");
		System.out.println(dto2);  // 어떤 클래스의 toString() 결과?? => 자식 재정의
	}
	
	// 부모타입의 다형성 : 모든 자식객체를 참조(저장) 정의 
	public static void main1(String[] args) {
		...
		// error : 다른 타입, 작은 타입, 즉 부모객체를 자식객체에 저장(참조) 불가 
